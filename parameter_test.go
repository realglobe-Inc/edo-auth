...
func newTestParameter() (*parameters, error) {
	file, err := ioutil.TempFile("", "edo-auth-proxy")
	if err != nil {
...
-rediUri=http://localhost:` + strconv.Itoa(test_socPort) + `/callback
-sigAlg=ES384
-sigKid=
-pathOk=/ok
-pathAuth=/
...
-pathCoop=/api
-tmplErr=
-asessLabel=Auth-User-Backend
-asessLen=30
-asessExpIn=1h
-asessDbExpIn=24h
-fsessLabel=Auth-User
-fsessLen=30
-fsessExpIn=168h
-csessLabel=Edo-Cooperation
-csessLen=30
-statLen=10
-noncLen=10
-tokTagLen=10
...
-idpDbTag2=idp
-asessDbType=memory
-asessDbAddr=localhost:6379
-asessDbTag=backend.asession
-tokDbType=memory
...
`)); err != nil {
	...
	param, err := parseParameters("edo-auth-proxy", "-c", file.Name())
	if err != nil {
		return nil, erro.Wrap(err)
	}
